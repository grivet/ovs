...
benchmark_mpsc_queue(void)
{
    ...
    for (i = 0; i < n_threads; i++) {
        threads[i] = ovs_thread_create("sc_queue_insert", mpsc_queue_insert_thread, &aux);
    }
...
benchmark_list(void)
{
    ...
    for (i = 0; i < n_threads; i++) {
        threads[i] = ovs_thread_create("locked_list_insert", locked_list_insert_thread, &aux);
    }
...
benchmark_guarded_list(void)
{
    ...
    for (i = 0; i < n_threads; i++) {
        threads[i] = ovs_thread_create("guarded_list_insert", guarded_list_insert_thread, &aux);
    }
