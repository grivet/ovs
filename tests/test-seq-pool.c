...
test_seq_pool_alloc_full_range(void)
{
    ...
    for (i = 0; i < N_IDS; i++) {
        ...
        assert(seq_pool_new_id(pool, 0, &id));
        /* No double alloc.*/
        assert(ids[id] == false);
        ids[id] = true;
    }
...
test_seq_pool_alloc_steal(void)
{
    ...
    for (i = 0; i < N - 1; i++) {
        ...
        assert(seq_pool_new_id(pool, 1, &id));
    }
...
test_seq_pool_alloc_monotonic(void)
{
    ...
    for (i = 0; i < N_IDS; i++) {
        assert(seq_pool_new_id(pool, 0, &ids[i]));
        assert(ids[i] == i);
    }
...
test_seq_pool_alloc_under_limit(void)
{
    ...
    for (limit = 1; limit < N_IDS; limit++) {
        ...
        for (i = 0; i < limit; i++) {
            assert(seq_pool_new_id(pool, 0, &ids[i]));
            assert(ids[i] < limit + SEQ_POOL_CACHE_SIZE);
        }
        for (i = 0; i < limit; i++) {
            seq_pool_free_id(pool, 0, ids[i]);
        }
...
benchmark_seq_pool(void)
{
    ...
    for (i = 0; i < n_threads; i++) {
        threads[i] = ovs_thread_create("seq_pool_alloc", seq_pool_thread, &aux);
    }
...
id_pool_thread(void *aux_)
{
    ...
    for (i = 0; i < n_ids_per_thread; i++) {
        ...
        assert(id_pool_alloc_id(aux->pool, &th_ids[i]));
...
benchmark_id_pool(void)
{
    ...
    print_result(" id_pool new");

    ovs_barrier_block(&barrier);

    print_result(" id_pool del");

    ovs_barrier_block(&barrier);

    print_result(" id_pool mix");

    for (i = 0; i < n_threads; i++) {
        xpthread_join(threads[i], NULL);
    }

    print_result(" id_pool rnd");
